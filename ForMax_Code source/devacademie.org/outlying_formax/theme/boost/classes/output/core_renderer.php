...
$context['email'] = $USER->email;
    ...
    public function firstview_fakeblocks(): bool {
        ...
        /**
     * Rendu des cours sous forme de liste horizontale.
     *
     * @param int $categoryid L'identifiant de la catégorie des cours.
     * @return string HTML contenant la liste des cours affichée horizontalement.
     */
    public function render_courses_horizontal($categoryid) {
        global $DB, $OUTPUT;

        // Récupérer les cours de la catégorie
        $courses = $DB->get_records('course', array('category' => $categoryid));

        // Générer l'affichage horizontal
        $content = '<div id="frontpage-course-list" class="courses-container" style="display: flex; flex-wrap: nowrap; overflow-x: auto; gap: 10px; padding: 10px;">';

        foreach ($courses as $course) {
            $courseurl = new moodle_url('/course/view.php', array('id' => $course->id));
            $coursename = format_string($course->fullname);
            
            // Ajout d'une carte pour chaque cours
            $content .= '<div class="course-card" style="flex: 0 0 auto; width: 250px; border: 1px solid #ddd; border-radius: 8px; padding: 10px; text-align: center; background: #f9f9f9;">';
            $content .= '<h3 style="font-size: 16px; margin-bottom: 10px;">' . $coursename . '</h3>';
            $content .= '<a href="' . $courseurl . '" style="display: inline-block; padding: 8px 12px; background: #007bff; color: white; text-decoration: none; border-radius: 5px;">Accéder au cours</a>';
            $content .= '</div>';
        }
